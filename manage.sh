...
case $ACTION in
    1)
        ...
        DESCRIPTIONS=(
            "[Загальні питання / Агенти] ⭐ Універсальний помічник"
            "[Програмування (Код)] ⭐ Найкраща для роботи в агентах VS Code"
            "[Складне кодування] Повільніша за Qwen 2.5 (частково в RAM)"
            "[Творчість / Чат] Висока якість мови та логіки"
            "[Альтернативний чат] Класична легка базова модель"
            "[Ультрашвидкі відповіді] Працює миттєво, мінімум ресурсів"
            "[Розпізнавання зображень] 👁️ Аналіз фотографій, складання описів"
            "[Деталізований Vision] 👁️ Неймовірна деталізація та OCR зображень"
